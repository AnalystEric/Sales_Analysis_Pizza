...
-- Q3: What is the total sold pizza throughout the year?
SELECT SUM(quantity) AS total_sold_pizza
FROM Pizza_DB.pizza_sales

-- Q4: What is the number of orders throughout the year?
SELECT COUNT(DISTINCT order_id) AS number_of_order
FROM Pizza_DB.pizza_sales

-- Q5: What is the total revenue in each month?
...
-- Q7: What is the number of orders throughout the day in each hour?
SELECT HOUR(order_time) AS hour, COUNT(DISTINCT order_id) AS num_of_orders
FROM Pizza_DB.pizza_sales
GROUP BY HOUR(order_time)
ORDER BY 1

-- Q8: What is the percentage of revenue in each pizza category?
...
-- Q9: What is the percentage of revenue in each pizza size aside from 'XL' and 'XXL'?
...
-- Q10: What is the most popular pizza by orders throughout the year?
...
-- Q11: What is the less popular pizza by orders throughout the year?
SELECT pizza_name
FROM Pizza_DB.pizza_sales
GROUP BY pizza_name
HAVING COUNT(DISTINCT order_id) = ( SELECT COUNT(DISTINCT order_id)
FROM Pizza_DB.pizza_sales
GROUP BY pizza_name
ORDER BY 1
LIMIT 1 ) 

-- Q12: What is the top 5 pizza by orders throughout the year?
